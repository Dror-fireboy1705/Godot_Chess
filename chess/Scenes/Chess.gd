...
func set_move(var2, var1):
	...
	if (selected_piece.x != var2 || selected_piece.y != var1) && ((white && board[var2][var1] > 0) || (!white && board[var2][var1] < 0)):
		...
		if white && is_in_check(white_king_pos): end_match(BLACK_CHECKMATE)
		elif !white && is_in_check(black_king_pos): end_match(WHITE_CHECKMATE)
		else: end_match(STALEMATE)
	
	if fifty_move_rule == 50: end_match(STALEMATE)
	if insufficient_material(): end_match(STALEMATE)
...
func threefold_position():
	for i in unique_board_moves.size():
		if board == unique_board_moves[i]:
			amount_of_same[i] += 1
			if amount_of_same[i] >= 3: end_match(STALEMATE)
			return
...
func end_match(message : CompressedTexture2D):
				win_message.texture = message
